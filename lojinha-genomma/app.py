...
def _find_excel_with_sheet(predicate, exclude=None):
    """Procura, entre todos os .xlsx do projeto, o primeiro cujas abas satisfaçam
    `predicate(sheetnames) -> bool`. Se `exclude` for informado, pula esse arquivo
    (usado para preferir um arquivo de preços dedicado em vez da aba de preços
    embutida na planilha de estoque). Não usa mtime como critério de desempate:
    depois de um deploy novo (git checkout), todos os arquivos do repositório
    recebem o mesmo horário de modificação, então mtime não serve para distinguir
    "o mais recente"."""
    ...
    for f in files:
        ...
        try:
            ...
            log.warning(f'⚠️  Não foi possível abrir "{f.name}" para inspeção: {e}')
            continue
        if predicate(sheetnames):
            return f
    return None
LOCK  = threading.Lock()
...
def _gh_backup_bytes(path, content_bytes):
    """Salva bytes (arquivo binário, ex.: comprovante/nota fiscal) em qualquer
    caminho do repositório GitHub. Usado para os uploads sobreviverem ao
    "sono" do plano free do Render, que apaga /tmp quando o serviço acorda
...
def _gh_restore_bytes(path):
    """Lê bytes (arquivo binário) de qualquer caminho do repositório GitHub."""
    if not GITHUB_TOKEN:
        return None
    ...
    if result and 'content' in result:
        try:
            ...
            log.info(f'☁️  Upload restaurado do GitHub: {path}')
            return data
        except Exception as e:
            log.warning(f'⚠️  Erro ao decodificar upload do backup: {e}')
    return None

def _save_upload(file, dest_path):
    """Salva o arquivo enviado (comprovante ou nota fiscal) localmente e, em
    paralelo, faz backup no GitHub (pasta backups/uploads/) — sem isso, o
    arquivo some quando o servidor reinicia (deploy novo ou o container
    "acordando" depois de dormir por inatividade no plano free do Render)."""
...
def load_from_excel():
    def _tem_aba_estoque(sheetnames):
        return any(n.strip().lower() == 'estoque' for n in sheetnames)
    def _tem_aba_precos(sheetnames):
        return any(n.strip().lower().startswith('tabela de venda') for n in sheetnames)

    excel_estoque = _find_excel_with_sheet(_tem_aba_estoque)
    if excel_estoque is None:
        raise FileNotFoundError(f'Nenhuma planilha com aba "Estoque" encontrada em {BASE}')
    log.info(f'📂 Planilha de estoque: {excel_estoque.relative_to(BASE)}')

    # Prefere um arquivo de preços dedicado (distinto do de estoque); se não existir
    # nenhum arquivo separado, cai de volta para a aba de preços embutida na própria
    # planilha de estoque (comportamento antigo, mantido como fallback).
    excel_precos = _find_excel_with_sheet(_tem_aba_precos, exclude=excel_estoque)
    if excel_precos is None:
        excel_precos = _find_excel_with_sheet(_tem_aba_precos)
    if excel_precos:
        log.info(f'📂 Planilha de preços: {excel_precos.relative_to(BASE)}')
    ...
    prices = {}
    if excel_precos:
        wb_p = openpyxl.load_workbook(str(excel_precos), read_only=True, data_only=True)
        price_sheet = next((s for s in wb_p.sheetnames if s.strip().lower().startswith('tabela de venda')), None)
        ws_tv = wb_p[price_sheet]
        ...
        wb_p.close()
        log.info(f'💲 {len(prices)} preços carregados da aba "{price_sheet}" ({excel_precos.name}).')
    else:
        log.warning('⚠️  Aba de preços ("Tabela de Venda(s)") não encontrada em nenhuma planilha.')

    # ── Lê estoque da aba "Estoque"
    wb = openpyxl.load_workbook(str(excel_estoque), read_only=True, data_only=True)
    ws = wb['Estoque']
...
def serve_upload(filename):
    ...
    if not local_path.exists():
        data = _gh_restore_bytes(f'backups/uploads/{filename}')
        if data:
            try:
                local_path.parent.mkdir(parents=True, exist_ok=True)
                local_path.write_bytes(data)
            except Exception as e:
                log.warning(f'⚠️  Falha ao restaurar upload do GitHub: {e}')
